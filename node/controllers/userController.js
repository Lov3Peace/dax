...
export const joinUserRoom = (socket) => {
  socket.on("joinUserRoom", (username) => {
    ...
    console.log(`Socket ${socket.id} joined user:${username} successfully`);
...
export const sendConnectionRequest = (socket, io) => {
  socket.on("sendConnectionRequest", async (data) => {
    try {
      ...
      const receiverUsernames = data.receivers;
      console.log(`data: ${data}`);
      console.log(
        `senderUsername: ${senderUsername}, receiverUsername: ${receiverUsernames}`,
      );

      console.log(receiverUsernames.length);
...
export const acceptConnectionRequest = (socket, io) => {
  socket.on("acceptConnection", async (data) => {
    try {
      ...
      io.to(`user:${connection.sender_username}`).emit(
        "connectionRequestAccepted",
        connection,
      );

      io.to(`user:${connection.receiver_username}`).emit(
        "connectionRequestAccepted",
        connection,
      );
    } catch (error) {
